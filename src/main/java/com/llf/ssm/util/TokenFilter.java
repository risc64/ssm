...
public class TokenFilter implements Filter {
	 ...
	 * 过滤器不过滤，如登录，查询，手册等
	 */
	private String[] exceptionPathArray;
	...
	public void doFilter(ServletRequest arg0, ServletResponse arg1, FilterChain arg2)
	    ...
	    for(int i = 0; i < exceptionPathArray.length; i++ ) {
    			int index = exceptionPathArray[i].indexOf("*");
	    		if(index != -1) {
	    			if(url.length() >= index) {
	    				String sub = url.substring(0, index);
	    				if(sub.equals(exceptionPathArray[i].substring(0, index))) {
		    				isException = true;
		    				break;
	    				}
	    			}
	    		} else {
	    			if(url.equals(exceptionPathArray[i])) {
	    				isException = true;
	    				break;
	    			}
	    		}	    		
	...
	public void init(FilterConfig arg0) throws ServletException {
		...
		String str = systemConfig.getExceptionPaths();
		exceptionPathArray = str.split(",");
